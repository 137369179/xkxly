...
function launchChrome() {
  return new Promise((resolve, reject) => {
    const chrome = spawn(CHROME, ['--headless=new', '--no-sandbox', '--disable-gpu', '--remote-debugging-port=0'], { stdio: ['ignore', 'pipe', 'pipe'] });
    let stderr = '';
...
class CdpSession {
  ...
  attach() {
    return this.send('Target.getTargets').then(async (targets) => {
      ...
      for (let i = 0; !page && i < 10; i++) { await sleep(500); const t = await this.send('Target.getTargets'); page = t.targetInfos.find((x) => x.type === 'page'); }
...
async function main() {
  ...
  for (const route of ROUTES) {
    ...
    console.log(`  [${tag}] /${route}/  root=${r.root}  mountCrash=${mountCrashes.length}  click=${ix.clicked ? 'Y' : 'n'} nav=${ix.navigated ? 'Y' : 'n'} ixCrash=${ix.crashes.length}${crashedUI ? ' CRASH-UI' : ''}`);
    mountCrashes.slice(0, 2).forEach((e) => console.log(`        mount - ${String(e).slice(0, 200)}`));
    ix.crashes.slice(0, 2).forEach((e) => console.log(`        click - ${String(e).slice(0, 200)}`));
    // 交互若导航走，回到本路由以免污染下一轮
    if (ix.navigated) await cdp.setHash(`#/${route}`);
  }

  await cdp.detach(); ws.close(); chrome.kill(); server.close();
  ...
  if (failRoutes.length) {
    console.log('异常路由：', failRoutes.map((r) => r.route).join(', '));
    process.exitCode = 2;
  }
}
