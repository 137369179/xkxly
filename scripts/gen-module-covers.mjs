 ...
 * 生产封面双源说明（重要）
 * ───────────────────────────────────────────────────────────────────────────
 *  本脚本产出的是「扁平色调块」封面（tone.main 底 + 白色舞台圆 + tone.deep 母题），
 *  作为无 AI 环境 / CI 的兜底与快速预览源。
 *  线上真实使用的封面为「AI 插画封面」，由 00010 电影编辑风工作流在 Ardot 画布生成
 *  （单一真相源见 scripts/gen-module-covers.mjs COVERS 的 id/tone 映射），导出 512²
 *  PNG 后按同名覆盖 public/icons/cover-<id>.png。两者 id / tone 必须严格一一对应，
 *  零图内文字，母题与模块一一对应（见《模块封面 · 插画落地规范》）。
 * ───────────────────────────────────────────────────────────────────────────
 */
