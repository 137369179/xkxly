...
function getLiushuInfo(char) {
  const liushuMap = {
    ...
    '读': { type: '形声字', desc: '言表意，卖表音', story: '左边是言字旁表示说话，右边是卖的读音' },
    '写': { type: '会意字', desc: '用手在屋内书写', story: '上面是宝盖头表示屋子，下面是与，表示在屋子里写字' }
  };
...
function generateSpeech(text, outputFile) {
  try {
    if (process.platform === 'darwin') {
      ...
      // Linux 使用 espeak
      execSync(`espeak -v zh -s 110 "${text}" -w "${outputFile}.wav" 2>/dev/null`, { stdio: 'pipe' });
...
async function generateVideo(h) {
  ...
  try {
    ...
    console.log(`  📝 ${char}: ${voiceText.substring(0, 40)}...`);
    ...
    const filterComplex = `[0:v]format=yuv420p,zoompan=z='min(zoom+0.0008,1.15)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${Math.floor(duration * 25)}:s=1280x720:fps=25,fade=t=in:st=0:d=0.5,fade=t=out:st=${Math.max(0, duration - 0.5)}:d=0.5[v]`;
    ...
    console.log(`  ✓ ${char} (${duration.toFixed(1)}秒, ${sizeMB}MB)`);
    return true;
  } catch (e) {
...
// 主函数
async function main() {
  console.log('🎬 开始生成汉字教学视频...\n');
  console.log('⏰ 预计耗时：每个视频约 10-30 秒\n');
