...
/**
 * 生成50秒长的汉字教学视频
 * 方法：Agnes AIGC生成18秒基础动画 + FFmpeg扩展至50秒
 */
...
async function generateVideo(h) {
  ...
  try {
    ...
    console.log(`  📝 ${char}: ${voiceText.substring(0, 60)}...`);
    ...
    // 获取音频时长
    const audioDurationOutput = execSync(`ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "${audioFile}"`, { encoding: 'utf-8', shell: true }).trim();
    const audioDuration = parseFloat(audioDurationOutput);
    
    // 计算需要的帧数（24fps，目标50秒）
    const targetDuration = 50;
    const totalFrames = Math.floor(targetDuration * 24);
    
    // 生成视频：缓慢缩放 + 淡入淡出效果
    const filterComplex = `[0:v]format=yuv420p,zoompan=z='min(zoom+0.0004,1.1)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=${totalFrames}:s=1280x720:fps=25,fade=t=in:st=0:d=2,fade=t=out:st=${targetDuration-2}:d=2[v]`;
    
    // 使用 ffmpeg 生成视频
    execSync(`ffmpeg -y -loop 1 -i "${imageUrl}" -i "${audioFile}" -filter_complex "${filterComplex}" -map "[v]" -map 1:a -c:v libx264 -preset medium -crf 23 -c:a aac -b:a 192k -t ${targetDuration} "${videoPath}"`, { 
      stdio: 'pipe' 
    });
    
    const sizeMB = existsSync(videoPath) ? Math.floor(readFileSync(videoPath).length / 1024 / 1024) : 0;
    console.log(`  ✓ ${char} (${sizeMB}MB, ~${targetDuration}秒)`);
    return true;
  } catch (e) {
...
// 主函数
async function main() {
  console.log('🎬 开始生成50秒汉字教学视频...\n');
  console.log('⏰ 预计耗时：每个视频约 15-30 秒\n');
