...
export function StarCounter({ count, className = '' }: { count: number; className?: string }) {
  ...
  return (
    <div className={`jelly-shine flex items-center gap-1.5 rounded-full bg-candy-yellow-soft px-3.5 py-2 shadow-candy-sm ${className}`} role="status" aria-label={t('stars.total', { count })}>
      <StarIcon size={20} />
