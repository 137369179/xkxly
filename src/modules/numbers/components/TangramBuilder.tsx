  ...
  return (
    <div className="space-y-4">
          ...
          {PUZZLES.map((p, i) => {
            ...
            return (
              <button
                ...
                className={`py-1.5 px-3 rounded-2xl text-xs font-black transition-all flex items-center gap-1 border-2 ${
                  isSel
      ...
      <div className="relative bg-gradient-to-b from-amber-50 via-sky-50 to-emerald-50 rounded-3xl border-4 border-amber-200 p-4 shadow-md min-h-[360px] flex flex-col justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 font-bold">
                {isFreeMode ? '点击选定形状，旋转并自由拼接' : currentPuzzle.intro}
              </p>
            </div>
          </div>
          <button
        ...
        {/* 目标轮廓与七巧板积木画布 (SVG 画板) */}
        <div className="relative mx-auto my-3 w-full max-w-[380px] h-[240px] bg-white/80 rounded-2xl border-2 border-dashed border-amber-300 shadow-inner overflow-hidden flex items-center justify-center">
        ...
        <div className="flex flex-wrap gap-2 items-center justify-between">
          <div className="flex items-center gap-1.5">
            <span className="text-xs font-black text-slate-600">旋转选定积木：</span>
            <button
              type="button"
              onClick={() => handleRotateSelected(-45)}
              disabled={!selectedPieceId}
              className="px-3 py-1.5 rounded-xl bg-white border-2 border-slate-200 text-xs font-black text-slate-700 disabled:opacity-40 hover:bg-slate-50"
            >
              ↺ 逆时针 45°
            </button>
            <button
          ...
          {!isFreeMode && !isCompleted && (
            <button
              ...
              className="px-5 py-2 rounded-2xl bg-gradient-to-r from-amber-400 to-orange-500 text-white font-black text-xs shadow-md hover:scale-[1.02] active:scale-[0.98] transition-all"
            >
              ✨ 吸附拼装 & 唤醒小动物
            </button>
          )}
