...
/**
 * React 钩子：某类学科的"小茜建议难度"。
 * 模块在生成题目时用它做默认难度，并允许孩子手动覆盖（仍是主人）。
 */
export interface AdaptiveDifficultyMeta {
  /** 当前是否仍由小茜自动调节（孩子尚未手动选档） */
  auto: boolean;
  /** 小茜当前建议档位（与 auto 无关，始终反映 DDA 实时推荐） */
  recommended: 1 | 2 | 3;
  /** 处于跟随模式，且小茜的最新建议与正在用的档位不一致（等待安全时机切换） */
  pending: boolean;
  /** 回到"跟随小茜"模式，并立即采用最新建议 */
  reset: () => void;
  /** 在安全边界（一轮做完、重新开局）调用：把小茜的最新建议应用上来 */
  syncNow: () => void;
}
