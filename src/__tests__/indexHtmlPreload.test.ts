 ...
 * 真实首屏 Hero 背景图是 `/hero_jelly.jpg`，未被 preload → 浪费带宽也不提 LCP。
 *
...
describe('index.html 首屏图片 preload（P0-#4）', () => {
  it('HomeHero 的实际首屏图确实为 /hero_jelly.jpg（前置契约，资产变更时此测试需同步）', () => {
    expect(homeHero).toContain('/hero_jelly.jpg');
  });

  it('应预加载首页实际 LCP 图，而非仅 og 用图（修复目标）', () => {
    const preloads = imagePreloads();
    // 修复后必须预加载真实首屏图
    expect(preloads).toContain('/hero_jelly.jpg');
